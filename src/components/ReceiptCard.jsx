...
function ReceiptCard({ receipt, onDelete, onUpdate }) {
    ...
    warrantyEnd.setMonth(warrantyEnd.getMonth() + Number(editedData.warrantyMonths))
        ...
        <div style={{ 
            ...
            <div style={{ flexGrow: 1 }}>
                {isEditing ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '10px' }}>
                        ...
                        <input 
                            ...
                            onChange={handleInputChange} 
                            style={{ padding: '6px', fontSize: '13px', borderRadius: '4px', border: '1px solid #ccc', backgroundColor: '#fff' }}
                        />
                        <input 
                            type="number" 
                            name="warrantyMonths" 
                    ...
                    <>
                        <h3 style={{ margin: '0 0 8px 0', fontSize: '18px', color: '#333', cursor: 'pointer' }} onClick={() => setShowDetails(true)}>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    ...
                    {isEditing ? (
                        <button onClick={handleSave} style={{ backgroundColor: '#2e7d32', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold', padding: '4px 10px' }}>
                            Save
                        </button>
                    ) : (
                        <button onClick={() => setIsEditing(true)} style={{ backgroundColor: 'transparent', color: '#0066cc', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold', padding: '4px 8px' }}>
                            Edit
                        </button>
                    )}

                    <button onClick={() => onDelete(receipt.id)} style={{ backgroundColor: 'transparent', color: '#d32f2f', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold', padding: '4px 8px' }}>
                        Delete
                    </button>
                </div>
            </div>
            ...
            {showDetails && (
                <div style={{
                    ...
                    <div style={{ 
                        ...
                        <div style={{ textAlign: 'left', width: '100%', fontSize: '16px', color: '#333', lineHeight: '1.5' }}>
                            <h2 style={{ marginTop: 0 }}>{editedData.productName}</h2>
                            <p><strong>Магазин:</strong> {editedData.store}</p>
                            <p><strong>Купено на:</strong> {new Date(editedData.purchaseDate).toLocaleDateString()}</p>
                            <p><strong>Гаранция:</strong> {editedData.warrantyMonths} месеца</p>
                            <p><strong>Изтича на:</strong> {warrantyEnd.toLocaleDateString()}</p>
                            <p><strong>S/N:</strong> {editedData.serialNumber || 'Няма'}</p>
                        </div>
                        <button 
